{
  description = "A flake for an ocaml c compiler";
    ...
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in
      {
        ...
        packages = {
          ...
          #     $ nix run -- <args?>
          #
          default = self.packages.${system}.c_compiler;

          c_compiler = ocamlPackages.buildDunePackage {
            pname = "c_compiler";
            version = "0.1.0";
            duneVersion = "3";
            src = sources.ocaml;
        ...
        #
        checks = {
          # Run tests for the `c_compiler` package
          c_compiler =
            let
            ...
            self.packages.${system}.c_compiler.overrideAttrs
              (oldAttrs: {
                name = "check-${oldAttrs.name}";
                doCheck = true;
                buildPhase = patchDuneCommand oldAttrs.buildPhase;
                checkPhase = patchDuneCommand oldAttrs.checkPhase;
                # skip installation (this will be tested in the `c-compiler-app` check)
        ...
        devShells = {
          default = legacyPackages.mkShell {
            ...
            # Tools from packages
            inputsFrom = [
              self.packages.${system}.c_compiler
            ];
          };
        };
      });
